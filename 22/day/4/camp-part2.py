...
with open("input.txt") as f:
    ...
    for line in blob:
        ...
        overlap = int(pair2[0]) <= int(pair1[0]) and int(pair1[0]) <= int(pair2[1])
        overlap |= int(pair2[0]) <= int(pair1[1]) and int(pair1[1]) <= int(pair2[1])
        a = int(pair1[0]) - int(pair2[0])
        b = int(pair1[1]) - int(pair2[1])
        overlap |= a == 0 or b == 0 or a / abs(a) != b / abs(b)
