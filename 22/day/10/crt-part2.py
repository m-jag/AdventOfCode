...
class CRT:
    def __init__(self, instructions):
        ...
        for instruction in instructions:
            self.cpu.processInstruction(instruction)
        self.buffer = [[], [], [], [], [], []]

    def run(self):
        while not self.cpu.isDone():
            self.drawBuffer()
            self.cpu.cycle()
        
    def drawBuffer(self):
        sprite = Position(self.cpu.registers["X"].value, 0)
        draw = Position(self.cpu.cycleCount % 40, int(self.cpu.cycleCount / 40) % 6)
        print("Draw Position: {:}".format(draw))
        print("Sprite Position: {:}".format(sprite))
        pixelLit = draw.x in range(sprite.x - 1, sprite.x + 2)
        if (pixelLit):
            self.buffer[draw.y].append("#")
        else:
            self.buffer[draw.y].append(" ") # To make it easier to read
            #self.buffer[draw.y].append(".")
